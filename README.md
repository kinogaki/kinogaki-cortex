...
| Exp AL | Giving the workspace something to do | 2026-06-26 | partial (win + honest negative) — the System-2 serial workspace REACHES a 2-hop target (acc 1.00) where System 1 and one-step deferral score 0.00 (trapped on the intermediate); reachability is real and new. But on a single deterministic chain it ties a blind "apply-twice" — the focus/inhibition machinery is for SELECTING among competing chains, which this probe lacks; next axis named: competing candidate chains | [link](https://cortex.kinogaki.com/blog/multi-step-workspace/) | [experiments/multi-step-workspace](experiments/multi-step-workspace) |
| Exp AM | Keeping track of what's going on | 2026-06-26 | negative — a persistent situation model (Chambers-Jurafsky event chains + Zwaan who/where/topic slots) does NOT predict over long spans: the +0.55 bpw "everywhere" win is pure smoothing repair — a STATIC frozen unigram beats the live situation by −0.07 bpw on the 99% non-backoff slice; the situation helps only the same 0.9% backoff slice (AC's law, third mechanism to land there). Lesson: measure a top-down prior against a STATIC prior, never against no prior | [link](https://cortex.kinogaki.com/blog/situation-model/) | [experiments/situation-model](experiments/situation-model) |
| Exp AN | Reading structure back out of a sum | 2026-06-26 | split — HRR role-filler decode WITH known roles = 100% (robust to 8 bound pairs, even D=512): compositional reading solved when structure is supplied; the blind RESONATOR (factor a product with unknown roles) FAILS at affordable dimension (≈0% all-F over a 4000 codebook at D ≤ 2048 — the capacity wall). Payoff: VSA-decode works GIVEN structure (AH supplies, AL manipulates) — don't factor blindly. (Analogy probe not recovered — follow-up) | [link](https://cortex.kinogaki.com/blog/reading-structure/) | [experiments/reading-structure](experiments/reading-structure) |
| Exp AO | Reaching back by the right cue | 2026-06-26 | win — content-addressable cue retrieval binds long-distance subject–verb agreement (subject ~15 words back, across a clause) to a correct-number antecedent 99.96% vs offset-attention's 65.26% (a wall past its modal offset); reproduces human agreement-attraction interference (0.00% → 2.11% only with a recent opposite-number distractor; fan divides activation). New primitive: long-distance binding by content cue + fan | [link](https://cortex.kinogaki.com/blog/cue-retrieval/) | [experiments/cue-retrieval](experiments/cue-retrieval) |
| Exp AP | Phrases that rhyme share their counts | 2026-06-26 | negative — permutation-bound + FlyHash phrase addressing: similar phrases pool counts (beats floored literal on 67% of unseen-phrase probes) AND preserves order (×2.29 under scramble vs bag's ×1.00), but FlyHash crosstalk loses the tail (aggregate ppl 1206 vs literal 831; poor exact memory 423 vs 2.2). Fix: use as a backoff layer under the literal table | [link](https://cortex.kinogaki.com/blog/phrases-that-rhyme/) | [experiments/phrases-that-rhyme](experiments/phrases-that-rhyme) |
